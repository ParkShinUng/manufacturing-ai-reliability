...
vibrationRms   = 2.2 · (0.35 + 0.65·r) · (1 + 6.0·h²)        ± N(0, 0.05)
...
| `OVERLOAD` | FR-002 | torque, current | load factor ×1.6 applied to torque and current |
| `COOLING_DEGRADATION` | FR-002 | `c` | `c(t) = min(0.9, t/T_cool)`, demo default `T_cool` 900 s |
...
| T9 | `RUNNING`/`DEGRADED`/`IDLE` | `FAULT` | any protective condition (§3.4) | auto | — | rate forced 0, AI suspended |
